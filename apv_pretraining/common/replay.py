...
class Replay:
    def __init__(
        ...
        self._ongoing = ongoing# what is ongoing?
        self._minlen = minlen
        self._maxlen = maxlen
    ...
    def add_step(self, transition, worker=0):
        ...
        :param transition:
        :param worker:
        :return: no return, but append a new transition to self._ongoing_eps
        '''
        episode = self._ongoing_eps[worker]
        # transition is a dictionary with 9 keys
        # 'reward', 'is_first', 'is_last', 'is_terminal', 'image', 'orientations', 'height', 'velocity', 'action'
    ...
    def _generate_datum(self, length):
...
class get_one_batch(object):
    def __init__(self, replay: Replay, batch, length):
        self._replay = replay
        self._batch = batch
        self._length = length

    def __call__(self, *args, **kwargs):
        one_batch = self._replay._generate_datum(self._length)
        for k, _ in one_batch.items():
            one_batch[k] = []

        for _ in range(self._batch):
            datum = self._replay._generate_datum(self._length)
        ...
        for k, v in one_batch.items():
            ...
            if torch.cuda.is_available():
                one_batch[k] = one_batch[k].cuda(1)
