...
class EnsembleRSSM(nn.Module):
    def __init__(
        ...
        self._cells = nn.ModuleList([GRUCell(self._hidden, self._deter, norm=True).cuda(1) for _ in range(self._rnn_layers)])
        # self._cell = GRUCell(self._deter, norm=True)
        self._cast = lambda x: x.type(torch.FloatTensor).cuda(1) if torch.cuda.is_available() else lambda x: x.type(torch.FloatTensor)

        # build blocks for img in
        self._imag_in_modules = OrderedDict()
        self._imag_in_modules["img_in"] = nn.Linear(self._deter+50, self._hidden)
        if self._norm != "none":
            self._imag_in_modules["img_in_norm"] = nn.LayerNorm(self._hidden)
        ...
        for k in range(self._ensemble):
            ...
            if self._norm != "none":
                module["img_out_norm" + str(k)] = nn.LayerNorm(self._hidden)
        ...
        if self._norm != "none":
            module["obs_out_norm"] = nn.LayerNorm(self._hidden)
    ...
    def zero_action(self, batch_size):
        ...
        if torch.cuda.is_available():
            act_zero = act_zero.cuda(1)
        return act_zero

    def forward(self, embed, is_first, state=None):
        swap = lambda x: torch.permute(x, (1, 0) + tuple(range(2, len(x.shape))))
        if state is None:
            state = self.initial(embed.size(0))
    ...
    def imagine(self, fake_data, state):
        swap = lambda x: torch.permute(x, (1, 0) + tuple(range(2, len(x.shape))))
        if state is None:
            state = self.initial(fake_data.size(0))
    ...
    def obs_step(self, prev_state, embed, is_first, sample=True):
        # sum_it = lambda x: torch.einsum('b,b...->b...', 1.0 - is_first.astype(x.dtype), x)
        # prev_action = sum_it(prev_action)
        # for k, v in prev_state.items():
        #     prev_state[k] = sum_it(v)
        (prev_state,) = tf_nest.map_structure(
            lambda x: torch.einsum("b,b...->b...", 1.0 - is_first.type(x.dtype), x),
            (prev_state,),
        )
        prior = self.img_step(prev_state, sample)
        # todo: revise it according to the github version
    ...
    def img_step(self, prev_state, sample=True):
        ...
        x = torch.concat((prev_stoch, self.zero_action(prev_stoch.size(0))), -1)
    ...
    def kl_loss(self, post, prior, forward, balance, free, free_avg):
        ...
        # sg = lambda x: [x[k] = v.detach().clone() for k,v in x.items()]
        free = torch.tensor(free).cuda(1) if torch.cuda.is_available() else torch.tensor(free)
...
class Encoder(nn.Module):
    def __init__(
        ...
        if len(self.mlp_keys) > 0:
            ...
            for i, width in enumerate(self._mlp_layers):
                ...
                if self._norm != "none":
                    self._mlp_modules["norm" + str(i)] = nn.LayerNorm(width)
    ...
    @property
    def output_dim(self):
        return self._mlp_layers[-1]*len(self.mlp_keys) + 2*2*384
...
class Decoder(nn.Module):
    def __init__(
        ...
        if self.mlp_keys:
            ...
            for key, shape in shapes.items():
                self._mlp_out[key] = nn.Linear(width, shape)
    ...
    def forward(self, features):
        ...
        if self.mlp_keys:
            ...
            mlp_dists = {}
            for k, v in self._mlp_out:
                mlp_out = torch.reshape(v(mlp_out), tuple(features.size()[:-1]) + tuple(shapes[k]))
                mlp_mean = self._mlp_out[k](mlp_out)
                mlp_dists[k] = Independent(Normal(mlp_mean, 1.0), shapes[k])
...
class MLP(nn.Module):
    def __init__(self, input_size, shape, layers, units, act="elu", norm="none", **out):
        ...
        for index in range(self._layers):
            self._hidden_modules["dense" + str(index)] = nn.Linear(input_size, self._units)
            if self._norm != "none":
...
class GRUCell(nn.Module):
    ...
    def get_initial_state(self, inputs=None, batch_size=None, dtype=None):
        ...
        def create_zeros(unnested_state_size):
            ...
            if torch.cuda.is_available(): init_state = init_state.cuda(1)
            return init_state
